...
<?php
	include('includes/database.php');
							...
							if ($error > 0){
									...
									VALUES ('$id','$location','$content','$time') ") or die (mySQLi_error($conn));
...
<?php
	include("includes/database.php");
			$query=mySQLi_query($conn,"SELECT * from post LEFT JOIN user on user.user_id = post.user_id order by post_id DESC") or die(mySQLi_error($conn));
			while($row=mySQLi_fetch_array($query)){
...
<?php
	include("includes/database.php");
			$comment=mySQLi_query($conn,"SELECT * from comments where post_id='$post_id' order by post_id DESC") or die(mySQLi_error($conn));
...
?>
		 ...
		 <form  method="POST">			
			<div class="comment-area">
						...
						<?php $image=mysqli_query($conn,"select * from user where user_id='$id'")or die(mysqli_error($conn));
